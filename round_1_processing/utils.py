...
def get_proficiency_level(
    ...
    sys_messages = [
        ...
        {
            "role": "user",
            "content": f"""Determine the appropriate proficency level based on the following description of a course: {course_title}, {course_description}, with the following course learning: {course_learning}.
        """,
        },
    ]

    response = client.chat.completions.create(model="gpt-4", messages=sys_messages)
...
def run_in_parallel(course_df, pe_knowledge_df, client):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(
                process_row, row, skill_info_dict, pe_knowledge_df, lock, client
            )
