...
def get_s3_client():
    ...
    try:
        ...
        # Test credentials with a lightweight operation
        client.list_buckets()
...
def parse_s3_path(s3_path):
    ...
    if not key:
        raise ValidationError("Key path cannot be empty")
...
def validate_file_size(size_bytes, max_size_mb=100):
    ...
    if size_bytes > max_size_bytes:
        raise ValidationError(f"File size ({size_bytes} bytes) exceeds limit ({max_size_mb}MB)")
