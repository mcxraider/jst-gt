...
sfw_raw_data_path = sfw_raw_data_path
sfw_raw_data_sheet = sfw_raw_data_sheet
...
checkpoint_file_path = f"log_files/{target_sector_alias}_arc_chart_ref_checkpoint.txt"
pickle_file_path = f"{r2_output_path}/{target_sector_alias}_gpt_output.pkl"


r2_raw_output_path = f"{r2_output_path}/{target_sector_alias}_course_skill_pl_rac_raw.csv"
r2_valid_output_path = f"{r2_output_path}/{target_sector_alias}_r2_valid_skill_pl_{timestamp}.csv"
r2_invalid_output_path = f"{r2_output_path}/{target_sector_alias}_r2_invalid_skill_pl_{timestamp}.csv"
all_valid_output_path = f"{r2_output_path}/{target_sector_alias}_all_valid_skill_pl_{timestamp}.csv"
...
target_df = data.copy()
print(f"The Course DF has {target_df.shape[0]} rows, with {target_df.unique_id.nunique()} unique courses.")
...
knowledge_df["Knowledge / Ability Items"] = knowledge_df["Knowledge / Ability Items"].fillna("")
knowledge_df.groupby(['skill_lower', 'Proficiency Level'], as_index=False).agg({'Knowledge / Ability Items': ', '.join})

# Preprocessing Courses
# Read Course information
course_raw_data = pd.read_csv(course_raw_data_path, usecols=course_raw_data_cols, low_memory=False)

# Read Course description
course_description_raw_data = pd.read_csv(course_descr_data_path, usecols=course_descr_cols, low_memory=False)

# Drop rows without information
course_description_raw_data = course_description_raw_data.dropna()
course_description_raw_data = course_description_raw_data.drop_duplicates(subset=["Course Reference Number"], keep="first")
...
merged_course_df = pd.merge(course_description_raw_data, course_raw_data[['Skill Title','Course Reference Number']], on='Course Reference Number', how='inner')

merged_course_df["course_text"] = merged_course_df['Course Title'] + " |: " + merged_course_df["About This Course"]  + " | " + merged_course_df["What You'll Learn"] # + " | " + merged_course_df["What You'll Learn"]
merged_course_df['unique_text'] = merged_course_df['course_text'] + merged_course_df['Skill Title']
...
# RAC Chart Reference
kb_dic = knowledge_df.groupby('skill_lower').apply(lambda x: x.groupby('Proficiency Level')['Knowledge / Ability Items'].apply(', '.join).reset_index().to_dict(orient="records"))
...
print(f"Number of untagged skills after R2: {sub_result_df[sub_result_df.proficiency_level_rac_chart==0].shape[0]}")
...
sanity_check_df = r2_tagged_cases.groupby(by="Skill Title")["proficiency_level_rac_chart"].agg(set).reset_index()
...
for skill, pl_list in zip(sanity_check_df['Skill Title'], sanity_check_df['proficiency_level_rac_chart']):
...
if len(pl_violation_dict_list) == 0:
    ...
    violation_filtered_df = pd.merge(r2_tagged_cases, pl_violation_df, left_on=["skill_lower","proficiency_level_rac_chart"], right_on=["skill_lower","invalid_pl"], how="outer", suffixes=("","_y"))
    ...
    for_sanity_check_df = pd.concat([r2_untagged_cases, r2_invalid_cases], ignore_index=True)
...
r2_valid_output = r2_valid_output.drop(columns=["proficiency_level_rac_chart","reason_rac_chart","confidence_rac_chart"])
...
merged_raw_course_df = pd.merge(original_crs_descr_df, course_raw_data[['Skill Title','Course Reference Number']], on='Course Reference Number', how='inner')
poor_dq_crs = merged_raw_course_df[~merged_raw_course_df['Course Reference Number'].isin(list(final_df['Course Reference Number']))]
print(f"There are {poor_dq_crs['Course Reference Number'].nunique()} poor quality courses.\n")
...
missing_content_df.to_csv(f"{r2_output_path}/{target_sector_alias}_missing_content_course.csv", index=False)
remaining_poor_dq_df.to_csv(f"{r2_output_path}/{target_sector_alias}_poor_content_quality_course.csv", index=False)
