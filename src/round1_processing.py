...
course_raw_data_path = (
    f"{input_data_path}/{target_sector_alias}_course_pl_tagging_cleaned.xlsx"
)
...
r1_raw_output_path = (
    f"{r1_output_path}/{target_sector_alias}_pl_tagging_raw_{timestamp}.csv"
)
r1_valid_output_path = (
    f"{r1_output_path}/{target_sector_alias}_r1_valid_skill_pl_{timestamp}.csv"
)
r1_invalid_output_path = (
    f"{r1_output_path}/{target_sector_alias}_r1_invalid_skill_pl_{timestamp}.csv"
)
sector_irrelevant_skill_path = (
    f"{r1_output_path}/{target_sector_alias}_irrelevant_skills.csv"
)
...
print(
    f"Knowledge DF has {knowledge_df.shape[0]} rows and {knowledge_df.shape[1]} columns.\n"
)

# Course-Skill Data
course_raw_data = pd.read_excel(
    course_raw_data_path, sheet_name=course_raw_data_sheet, usecols=course_raw_data_cols
)
course_raw_data = course_raw_data.drop_duplicates(
    subset=["Course Reference Number", "Skill Title"], keep="first"
)
course_raw_data = course_raw_data.dropna()
course_raw_data = course_raw_data.reset_index(drop=True)
course_raw_data["skill_lower"] = [
    x.lower().strip() for x in course_raw_data["Skill Title"]
]
print(
    f"Course raw DF has {course_raw_data.shape[0]} rows and {course_raw_data.shape[1]} columns.\n"
)

# Pure course data
course_description_raw_data = pd.read_excel(
    course_descr_data_path, usecols=course_descr_cols
)
...
course_raw_data["Sector Relevance"] = course_raw_data["skill_lower"].apply(
    lambda x: "Not in sector" if x in beyond_sector_skills else "In Sector"
)

# Save those "Not In Sector" for eyeballing
sector_irrelevant_skills = course_raw_data[
    course_raw_data["Sector Relevance"] == "Not in sector"
]
sector_irrelevant_skills = sector_irrelevant_skills.reset_index(drop=True)
print(
    f"Saving sector irrelevant skills file. There are {sector_irrelevant_skills.shape[0]} sector-irrelevant skills.\n"
)
...
results = run_in_parallel(
    merged_course_df, knowledge_df, GET_PROFICIENY_LEVEL_PROMPT, client
)
...
merged_final_df["proficiency_level"] = [
    int(x) for x in merged_final_df["proficiency_level"]
]
...
sanity_check_df = (
    tagged_cases.groupby(by="Skill Title")["proficiency_level"].agg(set).reset_index()
)
...
for skill, pl_list in zip(
    sanity_check_df["Skill Title"], sanity_check_df["proficiency_level"]
):
    valid_pl_list = list(
...
violation_filtered_df = pd.merge(
    tagged_cases,
    pl_violation_df,
    left_on=["skill_lower", "proficiency_level"],
